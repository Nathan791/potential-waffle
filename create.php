...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validation
    if (empty($name) || empty($lname) || empty($pnumber) || empty($email) || empty($password) || empty($role)) {
        ...
        $stmt = $connection->prepare("INSERT INTO shop (name, lname, pnumber, email, password, role) VALUES (?, ?, ?, ?, ?, ?)");
        if ($stmt === false) {
            ...
            $stmt->bind_param("ssisss", $name, $lname, $pnumber, $email, $hashed_password, $role);
...
<div class="register-container p-6 max-w-lg mx-auto">
    ...
    <form method="post" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" class="form-input form-control" id="lname" name="lname" value="<?= htmlspecialchars($lname) ?>" required>
        </div>
        
        <div>
            <label for="pnumber" class="block text-sm font-medium text-gray-700 mb-1">Phone Number:</label>
            <input type="text" class="form-input form-control" id="pnumber" name="pnumber" value="<?= htmlspecialchars($pnumber) ?>" required pattern="\d*">
        </div>
